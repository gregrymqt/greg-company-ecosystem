...
export const claimsService = {
  ...
  getSummary: async (): Promise<ClaimSummary> => {
    const response = await ApiService.get<ClaimSummary>("/claims/summary");
    return response;
  },

  // Listagem de Claims com risco (ClaimAnalyticsDTO)
  getAnalyticsList: async (): Promise<ClaimAnalytics[]> => {
    const response = await ApiService.get<ClaimAnalytics[]>("/claims/active");
    return response;
  },

  // Agrupamento por motivos (ClaimByReasonDTO)
  getClaimsByReason: async (): Promise<ClaimByReason[]> => {
    const response = await ApiService.get<ClaimByReason[]>("/claims/by-reason");
    return response;
  },
  ...
  syncToRows: async () => {
    ...
    return (await ApiService.post<{ body: string }>("/claims/sync-rows", {}));
  }
};
