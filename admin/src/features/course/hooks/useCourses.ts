...
import type { PaginatedResponse, CreateCourseData, UpdateCourseData } from '@/features/course/types/course-manager.types';
...
export const useCourses = () => {
    ...
    const fetchCourses = useCallback(async (page = 1, pageSize = 10) => {
        setLoading(true);
        try {
            const response = await adminCourseService.getAll({ pageNumber: page, pageSize }); // [cite: 47]
    ...
    const createCourse = async (data: CreateCourseData) => {
        setLoading(true);
        try {
            await adminCourseService.create(data); // [cite: 60]
    ...
    const updateCourse = async (id: string, data: UpdateCourseData) => {
        setLoading(true);
        try {
            await adminCourseService.update(id, data); // [cite: 63]
    ...
    const deleteCourse = async (id: string) => {
        ...
        setLoading(true);
        try {
            await adminCourseService.delete(id); // [cite: 66]
