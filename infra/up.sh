...
echo -e "\n${YELLOW}📦 [1/2] Compilando e gerando imagem: infra-backend:latest...${RESET}"
docker build -t infra-backend:latest ./backend

# Build do Go Worker (Email / Transcoding)
echo -e "\n${YELLOW}📦 [2/2] Compilando e gerando imagem: infra-go-worker:latest...${RESET}"
...
# 4. Aplicações Stateless (.NET API, Go Worker)
