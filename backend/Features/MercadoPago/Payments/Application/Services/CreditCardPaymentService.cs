...
namespace MeuCrudCsharp.Features.MercadoPago.Payments.Application.Services
{
    public class CreditCardPaymentService : ICreditCardPaymentService
    {
        ...
        )
        {
            if (request == null)
                throw new ArgumentNullException(
                    nameof(request),
                    "Os dados do pagamento n�o podem ser nulos."
                );
        ...
        )
        {
            ...
            {
                throw new ArgumentException("Dados do pagador (email, CPF) s�o obrigat�rios.");
            }
            ...
            try
            {
                if (string.IsNullOrEmpty(user.CustomerId))
                {
                    _logger.LogInformation(
                        "Usu�rio {UserId} n�o possui CustomerId. Criando novo cliente e cart�o.",
                        userId
                    );
                ...
                {
                    _logger.LogInformation(
                        "Usu�rio {UserId} j� possui CustomerId. Adicionando novo cart�o.",
                        userId
                    );
                ...
                var novoPagamento = new MeuCrudCsharp.Features.MercadoPago.Payments.Domain.Entities.Payment()
                {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Falha ao processar pagamento via MP para o usu�rio {UserId}.",
                    userId
                );
        ...
        )
        {
            ...
            var user = await _userRepository.GetByIdAsync(Guid.Parse(userId));
            if (user == null)
                throw new AppServiceException("Usu�rio n�o encontrado.");
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(user.CustomerId))
                {
                    ...
                    user.CustomerId = customerWithCard.CustomerId;
                    _userRepository.Update(user); // Marca para atualiza��o
                }
                else
                {
                    _logger.LogInformation(
                ...
                _logger.LogInformation(
                    "Fluxo de cria��o de assinatura conclu�do para o usu�rio {UserId}. ID da Assinatura: {SubscriptionId}",
                    userId,
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Erro inesperado no fluxo de cria��o de assinatura para o usu�rio {UserId}.",
                    userId
                );
