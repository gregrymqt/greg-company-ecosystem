...
public static class RabbitMqExtensions
{
    public static WebApplicationBuilder AddRabbitMqIntegration(this WebApplicationBuilder builder)
    {
        builder.Services.AddSingleton<IConnection>(sp =>
        {
            ...
            if (!string.IsNullOrWhiteSpace(rabbitMqSettings.ConnectionString))
            {
                factory.Uri = new Uri(rabbitMqSettings.ConnectionString);
                logger.LogInformation("Conectando ao RabbitMQ via ConnectionString/URI...");
            }
            else
            {
