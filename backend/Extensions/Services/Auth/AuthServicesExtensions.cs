...
public static class AuthServicesExtensions
{
    ...
    private static AuthenticationBuilder AddGoogleProvider(this AuthenticationBuilder authBuilder, IConfiguration configuration)
    {
        var googleSettings = configuration.GetSection("Google").Get<GoogleSettings>();

        if (googleSettings is null || string.IsNullOrEmpty(googleSettings.ClientId) || string.IsNullOrEmpty(googleSettings.ClientSecret))
        {
            throw new InvalidOperationException("Credenciais do Google não encontradas no arquivo de configuração.");
        }
    ...
    private static AuthenticationBuilder AddJwtBearerProvider(this AuthenticationBuilder authBuilder, IConfiguration configuration)
    {
        var jwtSettings = configuration.GetSection("Jwt").Get<JwtSettings>();

        if (jwtSettings?.Key is null)
            throw new InvalidOperationException("Chave de assinatura do JWT não encontrada no arquivo de configuração.");
