...
import type { WalletCard } from "../types/wallet.types";
...
export const useWallet = () => {
  ...
  const fetchCards = useCallback(async () => {
    setLoading(true);
    try {
      const data = await UserWalletService.getAllCards();
      setCards(data);
    } catch (error) {
  ...
  const addCard = async (cardToken: string): Promise<boolean> => {
    setLoading(true);
    try {
      const newCard = await UserWalletService.addCard({ cardToken });
  ...
  const removeCard = async (cardId: string) => {
    ...
    setLoading(true);
    try {
      await UserWalletService.deleteCard(cardId);
  ...
  return {
    ...
    addCard,
    removeCard,
    refresh: fetchCards, // Expõe para caso precise recarregar manualmente
  };
};
